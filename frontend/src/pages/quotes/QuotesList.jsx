...
const QuotesList = () => {
  ...
  const columns = [
    { header: 'Nº', cell: (row) => <span className="text-xs text-slate-600 dark:text-slate-400 font-mono">{row.id.substring(0,8)}</span> },
    ...
    <div>
      <div className="flex justify-between items-start mb-2">
        <div>
          ...
          <p className="text-xs text-slate-600 dark:text-slate-400">#{row.id.substring(0,8)} • {new Date(row.created_at).toLocaleDateString()}</p>
        </div>
      ...
      <div className="flex space-x-2 mt-4 pt-4 border-t border-slate-200 dark:border-slate-700">
        ...
        <button
          onClick={() => downloadPDF(row.id)}
          className="flex justify-center items-center px-3 bg-slate-100 dark:bg-slate-700 py-1.5 rounded-lg text-blue-400 text-sm"
        >
          <Download className="w-4 h-4" />
        </button>
      </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-600 dark:text-slate-400">Gestión de cotizaciones</p>
        </div>
        <button
      ...
      <div className="flex space-x-2 bg-white dark:bg-slate-800 p-2 rounded-xl border border-slate-200 dark:border-slate-700 overflow-x-auto">
        {['all', 'borrador', 'enviado', 'aprobado', 'rechazado', 'convertido_a_venta'].map(s => (
          <button
            key={s}
            onClick={() => setStatusFilter(s)}
            className={`px-4 py-2 rounded-lg text-sm font-medium capitalize whitespace-nowrap transition-colors ${
              statusFilter === s ? 'bg-slate-200 dark:bg-slate-600 text-white' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:bg-slate-700 hover:text-slate-800 dark:text-slate-200'
            }`}
          >
            {s.replace(/_/g, ' ')}
          </button>
      ...
      <Modal isOpen={isDetailOpen} onClose={() => setIsDetailOpen(false)} title={`Presupuesto #${quoteDetail?.id.substring(0,8)}`} maxWidth="max-w-2xl">
        {quoteDetail && (
          <div className="space-y-6">
            <div className="flex justify-between items-center bg-slate-100 dark:bg-slate-700 p-4 rounded-xl">
              <div>
                <p className="text-slate-600 dark:text-slate-400 text-xs">Estado Actual</p>
            ...
            <div>
              <h3 className="font-bold text-slate-900 dark:text-slate-100 mb-3 border-b border-slate-200 dark:border-slate-700 pb-2">Productos ({quoteDetail.items.length})</h3>
              <div className="space-y-2">
                {quoteDetail.items.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center text-sm border-b border-slate-200 dark:border-slate-700/50 pb-2 last:border-0">
                    <div>
                      <p className="font-medium text-slate-900 dark:text-slate-100">{item.product_name}</p>
                      <p className="text-xs text-slate-600 dark:text-slate-400">{item.quantity} {item.unit_type} x {formatCurrency(item.unit_price_applied)}</p>
                    </div>
